    ...
    private void assertContains(Collection<TrustmarkDefinitionDiffResult> results, TrustmarkDefinitionDiffType type, String locationRegex, String descriptionRegex) {
        try {
            if (results == null || results.size() == 0)
                fail("Expecting TD Diff results to contain Type[" + type + "], Location[" + locationRegex + "], Description[" + descriptionRegex + "] - but the results are empty!");
            ...
            if (!found)
                fail("Expecting TD Diff results to contain Type[" + type + "], Location[" + locationRegex + "], Description[" + descriptionRegex + "] - but the result was not found!");
