...
public class AbstractDocumentXmlDeserializer<T> extends AbstractDocumentInputDeserializer<T, Element, Element, Element> {
    ...
    public AbstractDocumentXmlDeserializer(Codec<T> _codec) { super(_codec); }
    ...
    @Override
    protected String getSourceType() { return SerializerXml.OUTPUT_MIME_FORMAT; }
    ...
    protected boolean isObjectElement(Element element) {
        if (this.isSequenceElement(element)) { return false; }
        if (element.elementIterator().hasNext()) { return true; }
